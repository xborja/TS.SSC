    ...
    public class ManageController : Controller
    {
        ...
        public async Task<ActionResult> Index(ManageMessageId? message)
        {
            ...
            var model = new IndexViewModel
            {
                ...
                TwoFactor = true,// await UserManager.GetTwoFactorEnabledAsync(userId),
                //Logins = null, // await UserManager.GetLoginsAsync(userId),
                BrowserRemembered = true //await AuthenticationManager.TwoFactorBrowserRememberedAsync(userId)
            };
            return View(model);
        }
        ...
        public async Task<ActionResult> ChangePassword(ChangePasswordViewModel model)
        {
            ...
            //Primero cargamos los datos del usuario
            Usuario usuario = new Usuario();
            //Se carga la información del usuario
            //usuario = UsuarioBL.GetByUserName(HttpContext.User.Identity.GetUserId(), model.OldPassword);
            usuario = UsuarioBL.GetByUserName(HttpContext.User.Identity.GetUserId());

            if (!Encriptacion.ValidatePassword(model.OldPassword, usuario.password, usuario.passwordHash))
            {
                usuario.userName = string.Empty;
            }
            if (string.IsNullOrEmpty(usuario.userName))
            {
                //ModelState.AddModelError(string.Empty, "La contraseña actual es incorrecta, por favor intente de nuevo");
            }
            else
            {
                usuario.password = model.ConfirmPassword;
                string saltpass = Encriptacion.GetRandomSalt(20);
                string hashpass = Encriptacion.HashPassword(model.ConfirmPassword, saltpass);
                usuario.passwordHash = saltpass;
                usuario.password = hashpass;
                usuario.UsuarioModif = usuario.userName;
                int resultado = UsuarioBL.UpdatePass(usuario);
                if (resultado == 1)
                {
